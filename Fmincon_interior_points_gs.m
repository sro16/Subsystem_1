...
%% Optimisation of the wheels using the interior points algorithm
...
% Upper and lower bounds (w, do, di)
lb = [0.01 0.1 0.03];
ub = [0.3 0.3775 0.3];
...
% Objective function
FUNCTION = @(x) (((((3./(2.*pi.*x(1))).*(Wc+Wp) + ((3.*p.*g./8).*(x(2).^2-x(3).^2))).*((1-poi1.^2)/E1 + (1-poi2.^2)/E2))./x(2)).^0.5).*(Wc + Wp + (pi.*p.*x(1).*g./4).*(x(2).^2-x(3).^2));

% Running the optimisation function with fmincon interior points
x = fmincon(FUNCTION,x0,A,b,Aeq,beq,lb,ub,nonlc);

% Display the solution
disp('1. Optimising Wheel Geometry: ');
disp(' ');
disp('Solution')
disp(['w = ' num2str(x(1)) ' (Width of the wheels)'])    % Width of the wheels 
disp(['do = ' num2str(x(2)) ' (Outer diameter of the wheels)'])
disp(['di = ' num2str(x(3)) ' (Inner diameter of the wheels)'])
disp(' ')
toc
disp(' ')
d = FUNC3(x(1),x(2),x(3),poi1,E1,p);
disp(['The provisional Rolling Resistance following optimisation of the wheen geometry is ' num2str(d) ' N']);


%% Optimising the Material of the wheels

% Parameters carried through from part 1
w = (x(1));
do = (x(2));
di = (x(3));
...
disp(['Rolling Resistance for PU = ' tppur ' N'])
k(1,2) = tppur1;
l(1,2) = "PU";
...
% Creating an array and plot of materials against their rolling resistance
names = {'PLA'; 'PU'; 'PS'; 'POM'; 'PMMA'; 'PET'; 'PET'; 'PC'; 'PA'; 'ABS'};
plot(k,'b.','MarkerSize',20);
title('Part 2: Rolling Resistance for Material Options', 'fontsize', 14);
xlabel('Material');
